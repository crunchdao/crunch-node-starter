...
def _find_templates_dir() -> Path:
    ...
    Priority:
    1. Bundled inside the installed package (``coordinator_node/_templates/``)
    2. Repo root (development mode — ``scaffold/`` and ``packs/`` at repo root)
    """
    pkg_dir = Path(__file__).resolve().parent.parent  # coordinator_node/
    ...
    raise FileNotFoundError(
        ...
        "Install coordinator-node from PyPI or run from a repo checkout."
    )
...
def list_packs() -> None:
    ...
    print()
    print("Usage: coordinator-node init my-challenge --pack <pack>")
