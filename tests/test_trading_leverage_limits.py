...
class TestLeverageLimits:
    def test_position_leverage_clamped(self):
        sim = TradingEngine(cost_model=ZERO_COST, max_position_leverage=2.5)
    ...
    def test_portfolio_leverage_clamped(self):
        sim = TradingEngine(cost_model=ZERO_COST, max_portfolio_leverage=5.0)
        now = datetime.now(UTC)
    ...
    def test_add_to_position_respects_limit(self):
        sim = TradingEngine(cost_model=ZERO_COST, max_position_leverage=2.0)
        now = datetime.now(UTC)
    ...
    def test_zero_leverage_after_clamping_skips_order(self):
        sim = TradingEngine(cost_model=ZERO_COST, max_portfolio_leverage=2.0)
        now = datetime.now(UTC)
